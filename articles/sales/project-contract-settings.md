---
title: Detalhes do cabeçalho para contratos baseadas em projeto
description: Este artigo fornece informações sobre os campos e informações sobre contratos baseados em projeto no Project Operations.
author: rumant
ms.date: 10/20/2020
ms.topic: article
...
| Empresa Proprietária | Guia **Resumo** | A entidade legal que será responsável pelos custos e receitas acumulados dos projetos associados a esse contrato de projeto. Quando um contrato é criado a partir de uma cotação, este campo é copiado do campo correspondente no registro de cotação. | A empresa proprietária equipara-se ao conceito de entidade legal no módulo **Gerenciamento e contabilidade de projeto** do Project Operations. Todos os custos e receitas acumulados neste projeto serão contabilizados na contabilidade da empresa proprietária. |
| Cliente | Guia **Resumo** | Uma referência à empresa do cliente ou registro de conta. Quando um contrato é criado a partir de uma cotação, este campo é copiado do campo correspondente no registro de cotação. | A moeda no contrato do projeto é padronizada com base na moeda do cliente. A moeda pode ser alterada antes de salvar o contrato. |
...
| Quantidade Registrada | Detalhes de uma linha baseada em projeto | Tempo: toda a quantidade de tempo nos dados reais de custo de hora para uma função no projeto que é mapeada para esta linha de contrato. Despesas: todos os valores para esta categoria de despesas nos custos reais das despesas no projeto são mapeadas para esta linha de contrato. |
