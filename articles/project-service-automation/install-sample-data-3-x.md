...
4. Se você não estiver usando o Spencer Low com o nome de usuário **spencerl**, você precisa atualizar um arquivo adicional. Abra o arquivo **DemoDataPreImportConfig.xml**, e depois encontre a tag **userstocreateandconfigure**. Atualize a tag **\<login\>** com o nome de usuário de seu usuário Spencer Low. Para detalhes adicionais, consulte [Notas técnicas](#technical-notes).
...
4. Se seu nome completo de usuário "Spencer Low" tiver um ID de usuário diferente de **"spencerl"**, então você precisa atualizar um arquivo adicional. Abra **DemoDataPreImportConfig.xml**, e encontre a tag **userstocreateandconfigure**. Atualize a tag **\<login\>** com o loginId (diferencia maiúsculas de minúsculas). 
...
- **\<EntitiesToEnableScheduling\>** especifica quais entidade devem ser habilitadas para Reserva no Agendamento do Microsoft Dynamics (também conhecido como Universal Resource Scheduling).
